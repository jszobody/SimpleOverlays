<div class="container mx-auto bg-white rounded-lg shadow-lg p-10">
    ...
    <div>
        ...
        <div class="flex items-center my-8">
            <div class="w-2/3 pr-4">
                <div style="background-image: url(/images/transparent-pattern.png)">
                    <img class="shadow border border-gray-100 cursor-pointer" src="{{ route('view-overlay', ['stack' => $stack, 'overlay' => $current]) }}">
                </div>
            </div>
            <div class="w-1/3 pl-4">
                <div class="font-bold text-gray-700">Next up...</div>
        ...
        <div class="flex my-8 overflow-x-auto border border-gray-300 px-2">
            @foreach($stack->overlays AS $overlay)
                <div class="{{ $current->id == $overlay->id ? 'bg-gray-300' : 'bg-white' }}">
                    <div wire:click="jump({{ $overlay->id }})" class="w-56 h-32 border m-2 flex-shrink-0 overflow-hidden text-xs p-2 cursor-pointer select-none bg-white {{ $current->id == $overlay->id ? 'border-blue-500' : 'border-gray-300' }}">
                        {{ $overlay->content }}
                    </div>
                </div>
            @endforeach
